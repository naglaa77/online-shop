...
if (isset($_POST['update'])) {
    ...
    mysqli_query($con,  $update); // order to connect to database and isert the info
    if (move_uploaded_file($image_location, 'images/' . $image_name)) { //  Moves an uploaded file to a images folder

        echo "<script>alert('produt is update')</script>";
    } else {
        echo "<script>alert('produt is not update')</script>";
    }
